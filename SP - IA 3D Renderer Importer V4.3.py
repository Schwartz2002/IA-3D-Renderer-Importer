...
# V4.3:
# -Fixed the points jumping to random place when having very low position values.
#   THANKS TO PlanariaLab FOR FINDING THE FIX TO THIS!
...
CRAFT_NAME = "IA 3D Renderer V4.3"
...
for x in range(len(points)):
    ...
    if CHECK_DEPTH:
        ...
        xmlFile.write(f"    <Setter variable=\"pointXRender{x}\" function=\"zoom*({points[x][0]}*pointsX_xMult+{points[x][1]}*pointsX_yMult+{points[x][2]}*pointsX_zMult)\" priority=\"0\" activator=\"renderPing\" />\n")
        xmlFile.write(f"    <Setter variable=\"pointYRender{x}\" function=\"zoom*({points[x][0]}*pointsY_xMult+{points[x][1]}*pointsY_yMult+{points[x][2]}*pointsY_zMult)\" priority=\"0\" activator=\"renderPing\" />\n")
...
for i in range(len(points)):
    xmlFile.write(f"&lt;color=#{str(points[i][3])}{{pointEnabled{i}?&quot;FF&quot;:&quot;00&quot;}}&gt;&lt;pos={{pointXRender{i};f}}px&gt;&lt;voffset={{pointYRender{i};f}}px&gt;.&#xD;&lt;br&gt;")
